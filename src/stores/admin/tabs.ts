...
export const useTabsStore = defineStore('tabsStore', () => {
  ...
  const getTabByTo = computed(() => (to: string) => {
    const foundTab = tabs.value.find((tab) => tab.to === to)
  ...
  async function addNewTab(rawTab: any) {
    ...
    // set the active tab
    return setActiveTab(newTab.name).then(() => {
      return newTab
    })
  }
  ...
  async function setOpenTabs(rawTabs: any[], openTabsArg: any[]) {
    ...
    let newRawTabs = rawTabs.filter((rawTab: any) =>
      openTabsArg.some((openTab: any) => rawTab['tabId'] === openTab['TabId'])
    )
